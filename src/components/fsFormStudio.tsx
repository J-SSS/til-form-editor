...
import FsHeader from './layout/fsHeader';
import LayoutIndex from './layout';
import FsLnb from './layout/fsLnb';
import FsMain from './layout/fsMain';
import FsRnb from './layout/fsRnb';
import SheetPreview from './layout/SheetPreview';
import { BorderMode, Cell, CellPosition, HeaderContextMenuState, HorizontalAlign, InspectedElement, LnbGroup, Selection, VerticalAlign } from './fsFormStudio.types';
import { createLnbCustomGroups, createLnbSelectGroups, lnbAutoGroups } from './config/lnbGroups';
import './fsFormStudio.scss';
...
type AutoElementCategory = 'drafter' | 'company' | 'document';
...
const htmlToPlainText = (value: string) =>
  value
    ...
    .replace(/&quot;/g, '"')
    .replace(/&#39;/g, "'");
const resolveAutoCategoryFromItemId = (itemId: string): AutoElementCategory | null => {
  if (itemId.startsWith('drafter-')) return 'drafter';
  if (itemId.startsWith('company-')) return 'company';
  if (itemId.startsWith('doc-')) return 'document';
  return null;
};
const resolveAutoCategoryFromType = (type: string): AutoElementCategory | null => {
  if (type === 'auto-drafter') return 'drafter';
  if (type === 'auto-company') return 'company';
  if (type === 'auto-document') return 'document';
  return null;
};
const getAutoCategoryLabel = (category: AutoElementCategory) => {
  if (category === 'drafter') return '기안자 정보';
  if (category === 'company') return '회사 정보';
  return '문서 정보';
};
const normalizeElementOptions = (options?: Array<{ id: string; label: string; isDefault?: boolean }>) => {
  ...
  const hasDefault = base.some((opt) => opt.isDefault);
  return hasDefault ? base : base.map((opt, idx) => ({ ...opt, isDefault: idx === 0 }));
};

const toElementHtml = (el: NonNullable<Cell['elements']>[number]) => {
  const displayLabel = escapeHtml(el.label || el.name || '');
  const autoCategory = resolveAutoCategoryFromType(el.type);
  if (autoCategory) {
    return `<span class="cell-element-node cell-element-node-auto cell-element-node-auto-${autoCategory}" contenteditable="false" data-element-id="${el.id}" data-element-label="${escapeHtml(el.label)}">${displayLabel || '자동입력'}</span>`;
  }
  ...
  if (el.type === 'text') control = `<input class="cell-native-control" type="text"${placeholderAttr} readonly tabindex="-1" />`;
    ...
    const options = normalizeElementOptions(el.options);
    const optionsHtml = options.length > 0
      ? options.map((opt) => `<option${opt.isDefault ? ' selected' : ''}>${escapeHtml(opt.label || opt.id)}</option>`).join('')
      : `<option>${displayLabel || '선택'}</option>`;
    control = `<select class="cell-native-control" disabled tabindex="-1">${optionsHtml}</select>`;
  }
...
const FsFormStudio: React.FC = () => {
  ...
  const [autoHelpElement, setAutoHelpElement] = useState<{ label: string; categoryLabel: string; category: AutoElementCategory } | null>(null);
  ...
  const [rowHeightModal, setRowHeightModal] = useState<{ rowIndex: number; value: string } | null>(null);
  const [colWidthModal, setColWidthModal] = useState<{ colIndex: number; value: string } | null>(null);
  const [borderLineWidth, setBorderLineWidth] = useState('1px');
  const [borderLineStyle, setBorderLineStyle] = useState('solid');
  const [borderLineColor, setBorderLineColor] = useState('#c8d5e8');
  ...
  const hasRnbChanges = useMemo(() => {
    ...
    return (
      ...
      JSON.stringify(normalizeElementOptions(origin.options)) !== JSON.stringify(normalizeElementOptions(inspectedElement.options))
    );
  ...
  const selectSingleRow = useCallback((rowIndex: number) => {
    ...
    setFocusedCell({ row: safeRow, col: 0 });
    setSelection({
      startRow: safeRow,
      startCol: 0,
      endRow: safeRow,
  ...
  const selectSingleCol = useCallback((colIndex: number) => {
    const safeCol = Math.max(0, Math.min(colIndex, colCount - 1));
    setFocusedCell({ row: 0, col: safeCol });
    setSelection({
      startRow: 0,
      startCol: safeCol,
  ...
  const insertRowAt = useCallback((insertAt: number) => {
    const safeInsert = Math.max(0, Math.min(insertAt, rowCount));
    setCells((prev) => {
      const cols = prev[0]?.length || 0;
      const createEmptyRow = (rowIndex: number): Cell[] =>
        Array.from({ length: cols }, (_, col) => ({
          id: `${rowIndex}-${col}`,
          value: '',
          rowSpan: 1,
      ...
      for (let oldRow = 0; oldRow < prev.length; oldRow++) {
        if (oldRow === safeInsert) {
          next.push(createEmptyRow(next.length));
        }
        const mapped = prev[oldRow].map((cell, oldCol) => {
          let nextCell: Cell = { ...cell };
          if (nextCell.mergedFrom && nextCell.mergedFrom.row >= safeInsert) {
            nextCell = {
              ...nextCell,
              mergedFrom: { ...nextCell.mergedFrom, row: nextCell.mergedFrom.row + 1 },
            };
          }
          return { ...nextCell, id: `${next.length}-${oldCol}` };
        });
        next.push(mapped);
      }
      if (safeInsert === prev.length) {
        next.push(createEmptyRow(next.length));
      }
      return next;
    });
    setRowHeights((prev) => {
      const next = [...prev];
      next.splice(safeInsert, 0, DEFAULT_ROW_HEIGHT);
      return next;
    });
    const focusRow = Math.min(safeInsert, rowCount);
    setFocusedCell({ row: focusRow, col: 0 });
    setSelection({ startRow: focusRow, startCol: 0, endRow: focusRow, endCol: Math.max(0, colCount - 1) });
    setEditingCell(null);
    setInspectedElement(null);
    setAutoHelpElement(null);
  ...
  const deleteRowAt = useCallback((targetRow: number) => {
    if (rowCount <= 1) return;
    const safeTargetRow = Math.max(0, Math.min(targetRow, rowCount - 1));
    setCells((prev) => {
      const nextRows: Cell[][] = [];
      for (let oldRow = 0; oldRow < prev.length; oldRow++) {
        if (oldRow === safeTargetRow) continue;
        const nextRow = prev[oldRow].map((cell, oldCol) => {
          let nextCell: Cell = { ...cell };
          if (nextCell.mergedFrom) {
            if (nextCell.mergedFrom.row === safeTargetRow) {
              nextCell = { ...nextCell, isMerged: false, mergedFrom: undefined, rowSpan: 1, colSpan: 1 };
            } else if (nextCell.mergedFrom.row > safeTargetRow) {
              nextCell = {
                ...nextCell,
                mergedFrom: { ...nextCell.mergedFrom, row: nextCell.mergedFrom.row - 1 },
              };
          ...
          if (!nextCell.isMerged && (nextCell.rowSpan || 1) > 1) {
            ...
            if (spanStart < safeTargetRow && spanEnd >= safeTargetRow) {
    ...
    setRowHeights((prev) => prev.filter((_, idx) => idx !== safeTargetRow));
    const nextRow = Math.max(0, Math.min(safeTargetRow, rowCount - 2));
    setFocusedCell({ row: nextRow, col: 0 });
    setSelection({ startRow: nextRow, startCol: 0, endRow: nextRow, endCol: Math.max(0, colCount - 1) });
    setEditingCell(null);
    setAutoHelpElement(null);
    setInspectedElement((prev) => {
      if (!prev) return prev;
      if (prev.row === safeTargetRow) return null;
      if (prev.row > safeTargetRow) return { ...prev, row: prev.row - 1 };
      return prev;
    });
    setHeaderContextMenu(null);
    setRowHeightModal((prev) => (prev && prev.rowIndex === safeTargetRow ? null : prev));
  }, [colCount, rowCount]);

  const insertColAt = useCallback((insertAt: number) => {
    const safeInsert = Math.max(0, Math.min(insertAt, colCount));
    setCells((prev) =>
      prev.map((row, rowIndex) => {
        const nextRow: Cell[] = [];
        for (let oldCol = 0; oldCol < row.length; oldCol++) {
          if (oldCol === safeInsert) {
            nextRow.push({
              id: `${rowIndex}-${nextRow.length}`,
              value: '',
              rowSpan: 1,
              colSpan: 1,
              isMerged: false,
          ...
          if (nextCell.mergedFrom && nextCell.mergedFrom.col >= safeInsert) {
            nextCell = {
              ...nextCell,
              mergedFrom: { ...nextCell.mergedFrom, col: nextCell.mergedFrom.col + 1 },
            };
          }
          nextRow.push({ ...nextCell, id: `${rowIndex}-${nextRow.length}` });
        }
        if (safeInsert === row.length) {
          nextRow.push({
            id: `${rowIndex}-${nextRow.length}`,
            value: '',
            rowSpan: 1,
    ...
    setColWidths(() => computeDistributedWidths(colCount + 1));
    const focusCol = Math.min(safeInsert, colCount);
    setFocusedCell({ row: 0, col: focusCol });
    setSelection({ startRow: 0, startCol: focusCol, endRow: Math.max(0, rowCount - 1), endCol: focusCol });
    setEditingCell(null);
    setInspectedElement(null);
    setAutoHelpElement(null);
    setHeaderContextMenu(null);
  ...
  const deleteColAt = useCallback((targetCol: number) => {
    if (colCount <= 1) return;
    const safeTargetCol = Math.max(0, Math.min(targetCol, colCount - 1));
    setCells((prev) =>
      prev.map((row, oldRow) => {
        const nextRow: Cell[] = [];
        for (let oldCol = 0; oldCol < row.length; oldCol++) {
          if (oldCol === safeTargetCol) continue;
          let nextCell: Cell = { ...row[oldCol] };
          if (nextCell.mergedFrom) {
            if (nextCell.mergedFrom.col === safeTargetCol) {
              nextCell = { ...nextCell, isMerged: false, mergedFrom: undefined, rowSpan: 1, colSpan: 1 };
            } else if (nextCell.mergedFrom.col > safeTargetCol) {
              nextCell = {
                ...nextCell,
                mergedFrom: { ...nextCell.mergedFrom, col: nextCell.mergedFrom.col - 1 },
              };
          ...
          if (!nextCell.isMerged && (nextCell.colSpan || 1) > 1) {
            ...
            if (spanStart < safeTargetCol && spanEnd >= safeTargetCol) {
    ...
    setColWidths(() => computeDistributedWidths(colCount - 1));
    const nextCol = Math.max(0, Math.min(safeTargetCol, colCount - 2));
    setFocusedCell({ row: 0, col: nextCol });
    setSelection({ startRow: 0, startCol: nextCol, endRow: Math.max(0, rowCount - 1), endCol: nextCol });
    setEditingCell(null);
    setAutoHelpElement(null);
    setInspectedElement((prev) => {
      if (!prev) return prev;
      if (prev.col === safeTargetCol) return null;
      if (prev.col > safeTargetCol) return { ...prev, col: prev.col - 1 };
      return prev;
    });
    setHeaderContextMenu(null);
    setColWidthModal((prev) => (prev && prev.colIndex === safeTargetCol ? null : prev));
  }, [colCount, rowCount]);

  // 현재 포커스된 행 1개 삭제 액션입니다.
  const handleDeleteRow = useCallback(() => {
    deleteRowAt(focusedCell.row);
  }, [deleteRowAt, focusedCell.row]);

  // 현재 포커스된 열 1개 삭제 액션입니다.
  const handleDeleteCol = useCallback(() => {
    deleteColAt(focusedCell.col);
  }, [deleteColAt, focusedCell.col]);
  ...
  // 셀 클릭 시 포커스/선택을 맞추고 FsRnb 선택 상태를 해제합니다.
    ...
    setCells(prev => {
      ...
      for (let row = minRow; row <= maxRow; row++) {
        for (let col = minCol; col <= maxCol; col++) {
          const target = next[row][col];
          target.borderWidth = borderLineWidth;
          target.borderStyle = borderLineStyle;
          target.borderColor = borderLineColor;
          if (mode === 'all') {
  ...
  }, [borderLineColor, borderLineStyle, borderLineWidth, selection]);

  const applyBorderStyleToSelection = useCallback((style: { width?: string; lineStyle?: string; color?: string }) => {
    const minRow = Math.min(selection.startRow, selection.endRow);
    const maxRow = Math.max(selection.startRow, selection.endRow);
    const minCol = Math.min(selection.startCol, selection.endCol);
    const maxCol = Math.max(selection.startCol, selection.endCol);
    setCells((prev) =>
      prev.map((row, rIdx) =>
        row.map((cell, cIdx) => {
          ...
          return {
            ...cell,
            borderWidth: style.width ?? cell.borderWidth,
            borderStyle: style.lineStyle ?? cell.borderStyle,
            borderColor: style.color ?? cell.borderColor,
          };
        })
      )
    );
  ...
  const handleElementClick = useCallback((e: React.MouseEvent, row: number, col: number, elementId: string) => {
    ...
    if (!target) return;
    const autoCategory = resolveAutoCategoryFromType(target.type);
    if (autoCategory) {
      setInspectedElement(null);
      setAutoHelpElement({
        label: target.label || '',
        categoryLabel: getAutoCategoryLabel(autoCategory),
    ...
    setInspectedElement({
      ...
      options: normalizeElementOptions(target.options),
    });
    setFocusedCell({ row, col });
    setSelection({ startRow: row, startCol: col, endRow: row, endCol: col });
  }, [cells]);
  ...
  // FsRnb 입력 변경은 임시 편집 상태(inspectedElement)에만 반영합니다.
          ...
          const nextElements = (cell.elements || []).map((el) =>
                el.id === inspectedElement.elementId
                  ? {
                      ...
                      options: normalizeElementOptions(inspectedElement.options),
                }
              : el
          );
  ...
  const handleSyncSelectItems = useCallback((items: Array<{ id: string; label: string; isDefault: boolean }>) => {
    setInspectedElement((prev) => {
      if (!prev || prev.type !== 'select') return prev;
      return { ...prev, options: normalizeElementOptions(items) };
    });
  }, []);
  ...
  }, [selectSingleCol, selectSingleRow]);

  const openRowHeightModal = useCallback((rowIndex: number) => {
    const safeRow = Math.max(0, Math.min(rowIndex, rowCount - 1));
    selectSingleRow(safeRow);
    setRowHeightModal({ rowIndex: safeRow, value: String(rowHeights[safeRow] ?? DEFAULT_ROW_HEIGHT) });
    setHeaderContextMenu(null);
  }, [rowCount, rowHeights, selectSingleRow]);

  const applyRowHeightFromModal = useCallback(() => {
    if (!rowHeightModal) return;
    const parsed = Number.parseInt(rowHeightModal.value, 10);
    const clamped = Number.isFinite(parsed)
      ? Math.max(10, Math.min(1000, parsed))
      : Math.max(10, Math.min(1000, rowHeights[rowHeightModal.rowIndex] ?? DEFAULT_ROW_HEIGHT));
    setRowHeights((prev) => {
      const next = [...prev];
      if (next[rowHeightModal.rowIndex] === undefined) return prev;
      next[rowHeightModal.rowIndex] = clamped;
      return next;
    });
    setRowHeightModal(null);
  }, [rowHeightModal, rowHeights]);

  const openColWidthModal = useCallback((colIndex: number) => {
    const safeCol = Math.max(0, Math.min(colIndex, colCount - 1));
    selectSingleCol(safeCol);
    setColWidthModal({ colIndex: safeCol, value: String(colWidths[safeCol] ?? MIN_COL_WIDTH) });
    setHeaderContextMenu(null);
  }, [colCount, colWidths, selectSingleCol]);

  const applyColWidthFromModal = useCallback(() => {
    if (!colWidthModal) return;
    const parsed = Number.parseInt(colWidthModal.value, 10);
    const clamped = Number.isFinite(parsed)
      ? Math.max(10, Math.min(1000, parsed))
      : Math.max(10, Math.min(1000, colWidths[colWidthModal.colIndex] ?? MIN_COL_WIDTH));
    setColWidths((prev) => {
      const next = [...prev];
      if (next[colWidthModal.colIndex] === undefined) return prev;
      next[colWidthModal.colIndex] = clamped;
      return next;
    });
    setColWidthModal(null);
  }, [colWidthModal, colWidths]);
  ...
  const addAutoElementToSelection = useCallback((itemId: string, label: string) => {
    const category = resolveAutoCategoryFromItemId(itemId);
    if (!category) return;
    const minRow = Math.min(selection.startRow, selection.endRow);
    const maxRow = Math.max(selection.startRow, selection.endRow);
    const minCol = Math.min(selection.startCol, selection.endCol);
  ...
  const lnbAutoInputGroups = useMemo(
    () =>
      lnbAutoGroups.map((group) => ({
        ...group,
        items: group.items.map((item) => ({
          ...item,
          action: () => addAutoElementToSelection(item.id, item.label),
        })),
      })),
    [addAutoElementToSelection]
  );
  ...
  return (
    <>
      <LayoutIndex
        ...
        header={<FsHeader onOpenDoc={() => setIsDocOpen(true)} onContextMenu={suppressContextMenu} />}
        lnb={
          <FsLnb
            lnbTab={lnbTab}
        ...
        main={
          <FsMain
            ...
            onHeaderAddRowAbove={(rowIndex) => insertRowAt(rowIndex)}
            onHeaderAddRowBelow={(rowIndex) => insertRowAt(rowIndex + 1)}
            onHeaderSetRowHeight={openRowHeightModal}
            onHeaderDeleteRow={deleteRowAt}
            onHeaderAddColLeft={(colIndex) => insertColAt(colIndex)}
            onHeaderAddColRight={(colIndex) => insertColAt(colIndex + 1)}
            onHeaderSetColWidth={openColWidthModal}
            onHeaderDeleteCol={deleteColAt}
        ...
        rnb={
          <FsRnb
            ...
            autoHelpElement={autoHelpElement}
            ...
            onBorderLineWidthChange={(value) => {
              setBorderLineWidth(value);
              applyBorderStyleToSelection({ width: value });
            }}
            onBorderLineStyleChange={(value) => {
              setBorderLineStyle(value);
              applyBorderStyleToSelection({ lineStyle: value });
            }}
            onBorderLineColorChange={(value) => {
              setBorderLineColor(value);
              applyBorderStyleToSelection({ color: value });
            }}
            onFieldChange={handleRnbFieldChange}
            onSyncSelectItems={handleSyncSelectItems}
      ...
      {rowHeightModal && (
        <div className="modal-overlay" onClick={() => setRowHeightModal(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3 className="modal-title">행 높이 설정</h3>
              <button className="modal-close" onClick={() => setRowHeightModal(null)} aria-label="닫기">×</button>
            </div>
            <div className="modal-body">
              <input
                className="main-tab-modal-input"
                ...
                min={10}
                max={1000}
                value={rowHeightModal.value}
                onChange={(e) => setRowHeightModal((prev) => (prev ? { ...prev, value: e.target.value } : prev))}
                autoFocus
              />
              <p className="modal-text">최소 10, 최대 1000</p>
              <div className="main-tab-modal-actions">
                <button
                  type="button"
                  className="main-tab-modal-button primary"
                  onClick={applyRowHeightFromModal}
                >
                  확인
                </button>
                <button
                  type="button"
                  className="main-tab-modal-button"
                  onClick={() => setRowHeightModal(null)}
                >
                  취소
                </button>
              </div>
      ...
      {colWidthModal && (
        <div className="modal-overlay" onClick={() => setColWidthModal(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3 className="modal-title">열 너비 설정</h3>
              <button className="modal-close" onClick={() => setColWidthModal(null)} aria-label="닫기">×</button>
            </div>
            <div className="modal-body">
              <input
                className="main-tab-modal-input"
                ...
                min={10}
                max={1000}
                value={colWidthModal.value}
                onChange={(e) => setColWidthModal((prev) => (prev ? { ...prev, value: e.target.value } : prev))}
                autoFocus
              />
              <p className="modal-text">최소 10, 최대 1000</p>
              <div className="main-tab-modal-actions">
                <button
                  type="button"
                  className="main-tab-modal-button primary"
                  onClick={applyColWidthFromModal}
                >
                  확인
                </button>
                <button
